...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // ID de la empresa a editar (deberías pasarlo como un campo oculto en el formulario)
    ...
    $sql = "UPDATE sg_omil_nuevasempresas SET RazonSocial=?, Giro=?, Rut=?, Direccion=?, Sector=?, TamañoEmpresa=?, AccesibilidadSucursal=?, ContactoCasaMatriz=?, TelefonoCasaMatriz=?, CorreoCasaMatriz=?, NombreContacto=?, CargoContacto=?, TelefonoContacto=?, CorreoContacto=? WHERE IdentificacionEmpresa=?";

    // Preparar la consulta
    $stmt = $conn->prepare($sql);

    // Vincular los parámetros
    $stmt->bind_param("ssssssssssssssi", $razon_social, $giro, $rut, $direccion, $sector, $tamanio_empresa, $accesibilidad_sucursal, $contacto_casa_matriz, $telefono_casa_matriz, $correo_casa_matriz, $nombre_contacto, $cargo_contacto, $telefono_contacto, $correo_contacto, $empresa_id);
...
<body>
    ...
    <a href="admin.php">Volver a la página de administración</a>
</body>
</html>
